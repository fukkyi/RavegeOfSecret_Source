...
public class MultipleTrackingProvider : BackgroundDataProvider
{
    bool readFirstFrame = false;
    TimeSpan initialTimestamp;

    public MultipleTrackingProvider(int id) : base(id)
    {
    ...
    protected override void RunBackgroundThreadAsync(int id, CancellationToken token)
    {
        try
        {
            ...
            {
                device.StartCameras(SelectMatchConfiguration(device));

                UnityEngine.Debug.Log("Open K4A device successful. id " + id + "sn:" + device.SerialNum);

                //TrackingManager.Instance.SetDebugUIKinectState("Open Successful");
    ...
    /// <returns></returns>
    protected DeviceConfiguration SelectMatchConfiguration(Device device)
    {
        ...
        if (device.SyncInJackConnected && device.SyncOutJackConnected)
        {
